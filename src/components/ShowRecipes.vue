<script lang="ts">
import type { PropType } from 'vue';
import MainButton from './MainButton.vue';
import type { TRecipe } from '@/types/TRecipe';
import { itensInList1AreInList2 } from '@/operations/lists';
import RecipeCard from './RecipeCard.vue';


export default {
    components: {MainButton, RecipeCard},
    props: {
        ingredientes: { type: Array as PropType<string[]>, required: true }
    },
    emits: ['editarIngredientes'],
    data(){
        return {
            foundRecipes: [] as TRecipe[]
        }
    },
    async created(){
        const recipes: TRecipe[] = await this.fetchRecipes();

        this.foundRecipes = recipes.filter((recipe) => {
            const canMakeRecipe = itensInList1AreInList2(recipe.ingredientes, this.ingredientes);

            return canMakeRecipe
        })
    },
    methods: {
        async fetchRecipes<TRecipe>(){
            const url = 'https://gist.githubusercontent.com/antonio-evaldo/002ad55e1cf01ef3fc6ee4feb9152964/raw/bf463b47860043da3b3604ca60cffc3ad1ba9865/receitas.json';

            const recipes = await fetch(url);

            return recipes.json() as TRecipe
        }
    }
}
</script>

<template>
    <section class="mostrar-receitas">
        <h1 class="cabecalho titulo-receitas">Receitas</h1>
        
        <p class="paragrafo-lg resultados-encontrados">
            Resultados encontrados: {{ foundRecipes.length }}
        </p>
        
        <div v-if="foundRecipes.length" class="receitas-wrapper">
            <p class="paragrafo-lg informacoes">
                Veja as opções de receitas que encontramos com os ingredientes que você tem por aí!
            </p>
            
            <ul class="receitas">
                <li v-for="receita of foundRecipes" :key="receita.nome">
                    <RecipeCard :receita="receita" />
                </li>
            </ul>
        </div>
        
        <div v-else class="receitas-nao-encontradas">
            <p class="paragrafo-lg receitas-nao-encontradas__info">
                Ops, não encontramos resultados para sua combinação. Vamos tentar de novo?
            </p>
            
            <img src="../assets/imagens/sem-receitas.png"
            alt="Desenho de um ovo quebrado. A gema tem um rosto com uma expressão triste.">
        </div>
        
        <MainButton text="Editar Ingredientes" @click="$emit('editarIngredientes');" />
    </section>
  </template>
  
  <style scoped>
  .mostrar-receitas {
    display: flex;
    flex-direction: column;
    align-items: center;
    text-align: center;
  }
  
  .titulo-receitas {
    color: var(--verde-medio, #3D6D4A);
    margin-bottom: 1.5rem;
  }
  
  .resultados-encontrados {
    color: var(--verde-medio, #3D6D4A);
    margin-bottom: 0.5rem;
  }
  
  .receitas-wrapper {
    margin-bottom: 3.5rem;
  }
  
  .informacoes {
    margin-bottom: 2rem;
  }
  
  .receitas {
    display: flex;
    justify-content: center;
    gap: 1.5rem;
    flex-wrap: wrap;
  }
  
  .receitas-nao-encontradas {
    margin-bottom: 2rem;
  }
  
  .receitas-nao-encontradas__info {
    margin-bottom: 0.5rem;
  }
  
  @media only screen and (max-width: 767px) {
    .receitas-wrapper {
      margin-bottom: 2rem;
    }
  }
  </style>
  